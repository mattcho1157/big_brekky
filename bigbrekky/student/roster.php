	...
	<div class="container">
		...
		<div class="collapse navbar-collapse" id="navbar">
			<ul class="nav navbar-nav">
				...
				<li><a href="roster.php" id="current-page">ROSTER</a></li>
...
<div class="page-content">
	...
	<div class="panel-group panel-group-students" id="accordion">
	<?php 
	//get all events that the student is allocated to
	$studentEvents = DB::query('select s.eventid, e.* from studentevents s left join events e on s.eventid = e.eventid where s.username = %s order by e.eventdate', $_SESSION['username']);
	for ($i=0; $i<count($studentEvents); $i++) {
		$event = $studentEvents[$i];
		echo '
		<div class="panel panel-default">
		<div class="panel-heading">
			<h3 class="panel-title">
			<a data-toggle="collapse" data-parent="#accordion" href="#collapse'.$i.'">Week '.$event['week'].' '.strtoupper($event['day']).'</a>
			</h3>
		</div>
		<div id="collapse'.$i.'" class="panel-collapse collapse">
			<div class="panel-body">
				<p>DATE: '.date_format(date_create($event['eventdate']), 'j M Y').'</p>
